...
namespace BSP::Motor::DM
{
    ...
    public:
         ...
         * @brief DM电机的MIT控制方法
         */
        void ctrl_Mit(CAN_HandleTypeDef *hcan, uint8_t id, float _pos, float _vel, 
         ...
         * @brief DM电机的角度速度控制方法
         */
        void ctrl_AngleVelocity(CAN_HandleTypeDef *hcan,uint8_t id, float _pos, float _vel)
        {
            uint8_t data[8];
            uint8_t *pbuf, *vbuf;
         ...
         * @brief DM电机的速度控制方法
         */
        void ctrl_Velocity(CAN_HandleTypeDef *hcan,uint8_t id, float _vel)
        {
            uint8_t data[8] = {0};
         ...
         */
        void On(CAN_HandleTypeDef *hcan,uint8_t id, Model mod)
        {
         ...
         * @param mod 模式可以有3种: MIT = 0, ANGLEVELOCITY = 1, VELOCITY = 2
         */
        void Off(CAN_HandleTypeDef *hcan, uint8_t id, Model mod)
        {
         ...
         * @param mod 模式可以有3种: MIT = 0, ANGLEVELOCITY = 1, VELOCITY = 2
         */
        void SetZero(CAN_HandleTypeDef *hcan,uint8_t id, Model mod)
        {
            uint8_t send_data[8] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFE};
         ...
         * @param mod 模式可以有3种: MIT = 0, ANGLEVELOCITY = 1, VELOCITY = 2
         */
        void ClearErr(CAN_HandleTypeDef *hcan, uint8_t id, Model mod)
        {
     ...
     */
    template <uint8_t N> 
    class J4340P : public DMMotorBase<N>
    {
    public:
        J4340P(uint16_t Init_id, const uint8_t (&ids)[N], const uint32_t (&send_idxs_)[N])
            : DMMotorBase<N>(Init_id, ids, send_idxs_, 
                            Parameters(-3.14f, 3.14f, -10.0f, 10.0f, -28.0f, 28.0f, 0.0f, 500.0f, 0.0f, 5.0f))
        {
        }
    };
     ...
     */
    template <uint8_t N> 
    class J8009P : public DMMotorBase<N>
    {
    public:
        J8009P(uint16_t Init_id, const uint8_t (&ids)[N], const uint32_t (&send_idxs_)[N])
            : DMMotorBase<N>(Init_id, ids, send_idxs_, 
                            Parameters(-3.14f, 3.14f, -45.0f, 45.0f, -54.0f, 54.0f, 0.0f, 500.0f, 0.0f, 5.0f))
        {
        }
    };
} // namespace BSP::Motor::DM
