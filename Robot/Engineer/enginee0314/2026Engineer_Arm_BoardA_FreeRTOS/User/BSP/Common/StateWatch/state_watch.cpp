...
{
    int32_t diff = (int32_t)(update_time_ - last_update_time_);
    
    // 如果diff < 0，说明 last_update_time_ 在 update_time_ 取样之后才被中断更新（发生了读写竞争倒挂）。
    // 这代表着设备刚刚正好发了最新的数据包来，故当前离上一次发包的流失时间记作0。
    if (diff < 0)
    {
        TimeThreshold_ = 0;
    }
